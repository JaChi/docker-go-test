...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		welcome := Welcome{"Mifman", time.Now().Format(time.Stamp)}
	...
	//Start the web server, set the port to listen to 80. Without a path it assumes localhost
	//Print any errors from starting the webserver using fmt
	fmt.Println("Listening on Port 3535")
	fmt.Println(http.ListenAndServe(":3535", nil))
}
